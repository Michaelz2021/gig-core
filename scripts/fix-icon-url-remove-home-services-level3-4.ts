...
async function fixIconUrls() {
  ...
  try {
    ...
    // Update category_level IN (3, 4): remove "Home+Services/" from icon_url
    const result = await client.query(
      `UPDATE service_categories
       SET icon_url = REPLACE(icon_url, $1, $2), updated_at = NOW()
       WHERE category_level IN (3, 4)
         AND icon_url LIKE $1 || '%'
       RETURNING id, name, category_level, icon_url`,
      [PREFIX_TO_REMOVE, NEW_PREFIX],
    );

    const updated = result.rowCount ?? 0;
    console.log(`Updated ${updated} rows (category_level 3 or 4).\n`);

    if (result.rows.length > 0) {
      console.log('Updated icon_url values:');
      result.rows.forEach((r: { name: string; category_level: number; icon_url: string }) => {
        console.log(`  [level ${r.category_level}] ${r.name} -> ${r.icon_url}`);
      });
    }
  } catch (error) {
