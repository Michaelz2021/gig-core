...
# Script to create user_bank_accounts table
...
SQL_FILE="$SCRIPT_DIR/create-user-bank-accounts-table.sql"
...
if [ $? -eq 0 ]; then
  echo ""
  echo "✅ user_bank_accounts table created successfully!"
else
  echo ""
  echo "❌ Error creating user_bank_accounts table"
  exit 1
fi
